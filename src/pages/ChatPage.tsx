...
export default function ChatPage() {
  ...
  // WebSocket for chat updates — connect once, don't reconnect on chat selection
  useEffect(() => {
    const disconnect = connectChat();
    return () => { disconnect(); };
  }, []);

  // Listen for new messages — use a ref for selectedChatId to avoid re-subscribing
          ...
          prev.map((c) =>
            c.id === event.chat_id
              ? {
                  ...
                  unread_count: c.id === selectedChatIdRef.current ? 0 : c.unread_count + 1,
                }
              : c
          )
        );
              ...
              >
                ...
                <div style={chatInfoStyle}>
                  <div style={chatNameStyle}>
                    {chat.order_title || `Заказ: ${chat.order_id.substring(0, 8)}`}
                  </div>
                  {chat.last_message && (
